...
export default function History() {
  ...
  const fetchHistory = async () => {
    ...
    try {
      ...
      setError(err.message || 'Lỗi khi kết nối với Supabase');
    } finally {
      setLoading(false);
    }
  };
  ...
  const clearHistory = async () => {
    ...
    try {
      ...
      if (error) {
        alert('Không thể xóa. Bạn cần thêm policy DELETE trong Supabase. Vui lòng xem hướng dẫn nếu bị lỗi.');
      } else {
        setRecords([]);
      }
    } catch (err) {
  ...
  if (error) {
    return (
      <div className="border border-red-900/50 border-dashed bg-red-950/20 p-8 flex flex-col items-center justify-center mx-auto w-full max-w-4xl text-center">
        ...
        <div className="bg-slate-950 p-6 border border-slate-800 text-left w-full">
          <h4 className="text-emerald-500 text-xs font-bold uppercase tracking-widest mb-2 border-b border-slate-800 pb-2">HƯỚNG DẪN KHẮC PHỤC</h4>
          <p className="text-slate-400 text-xs font-mono mb-4">
            Hệ thống chưa tìm thấy bảng phân phối khóa trong Supabase. Vui lòng thiết lập bảng bằng cách truy cập SQL Editor trên Supabase và chạy mã sau:
          </p>
          <pre className="text-[10px] text-emerald-300 font-mono bg-slate-900 p-4 border border-emerald-900/30 overflow-x-auto whitespace-pre-wrap">
...
CREATE POLICY "Allow anonymous delete" ON simulation_sessions FOR DELETE TO anon USING (true);`}
          ...
          <button 
            ...
            className="mt-4 bg-slate-800 hover:bg-slate-700 text-emerald-500 font-bold py-2 px-6 uppercase tracking-wider text-xs border border-emerald-900 flex mx-auto transition-colors"
          >
            TẢI LẠI DỮ LIỆU
          </button>
        </div>
        ...
        <table className="w-full text-xs text-left font-mono">
          ...
          <tbody>
            {loading && records.length === 0 ? (
               <tr>
                 <td colSpan={6} className="py-8 text-center text-emerald-600 opacity-50">ĐANG ĐỒNG BỘ CƠ SỞ DỮ LIỆU...</td>
               </tr>
            ) : records.length === 0 ? (
               <tr>
                 <td colSpan={6} className="py-8 text-center text-slate-500">CHƯA CÓ LƯỢT MÔ PHỎNG NÀO TRONG PHIÊN LÀM VIỆC.</td>
               </tr>
            ) : records.map((record) => (
